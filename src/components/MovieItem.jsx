    ...
    <li className="flex items-start bg-[#070D23] mt-4 pb-4 border-b border-b-[1px] border-[rgba(34,44,79,0.5)]  last:border-b-0">
      ...
      <div className="ml-4 flex-1 flex flex-col ">
        <div className="flex justify-between">
          <Link to={`/detail/${movie.id}`} className="flex-1">
            <h3 className="text-white text-[24px] leading-tight mb-0">{movie.title}</h3>
            <p className="text-gray-500 text-xs">{movie.genres?.join(",") || "unKnown genre"}</p>
          </Link>
          <button 
            onClick={handleLike}
            className="p-1"
        ...
        <p 
          className="opacity-80 text-[18px] font-thin mt-[6px] text flex items-center gap-1"
          style={{ color:'rgba(255, 255, 255, 1)'}}
        >
          {movie.year || "N/A"} • {movie.country || "unKnown"} • <img src={StarIcon} alt="star" className="w-4 h-4" />{movie.imdb_rating} 
        </p>
      </div>
    </li>
  );
